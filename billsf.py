...
class Bill_App:
    def __init__(s,t):
        ...
        #appliance
        f4=LabelFrame(s.t,text="Electric Appliance",bd=7,relief=GROOVE,bg="#074463",fg="gold",font=("times new roman",15,"bold"))
        f4.place(x=670,y=170,width=325,height=380)
        u1=Label(f4,text="AC",bg="#074463",fg="lightgreen",font=("times new roman",16,"bold")).grid(row=0,column=0,padx=10,pady=10,sticky="w")
        e1=Entry(f4,width=10,font="arial 16",bd=5,relief=SUNKEN,textvariable=s.c1).grid(row=0,column=1,padx=10,pady=10)
        u2=Label(f4,text="Refrigerator",bg="#074463",fg="lightgreen",font=("times new roman",16,"bold")).grid(row=1,column=0,padx=10,pady=10,sticky="w")
        e2=Entry(f4,width=10,font="arial 16",bd=5,relief=SUNKEN,textvariable=s.c2).grid(row=1,column=1,padx=10,pady=10)
        u3=Label(f4,text="TV",bg="#074463",fg="lightgreen",font=("times new roman",16,"bold")).grid(row=2,column=0,padx=10,pady=10,sticky="w")
        e2=Entry(f4,width=10,font="arial 16",bd=5,relief=SUNKEN,textvariable=s.c3).grid(row=2,column=1,padx=10,pady=10)
        u4=Label(f4,text="Water Heaters",bg="#074463",fg="lightgreen",font=("times new roman",16,"bold")).grid(row=3,column=0,padx=10,pady=10,sticky="w")
        e4=Entry(f4,width=10,font="arial 16",bd=5,relief=SUNKEN,textvariable=s.c4).grid(row=3,column=1,padx=10,pady=10)
        u5=Label(f4,text="Microwave Ovens",bg="#074463",fg="lightgreen",font=("times new roman",16,"bold")).grid(row=4,column=0,padx=10,pady=10,sticky="w")
        e5=Entry(f4,width=10,font="arial 16",bd=5,relief=SUNKEN,textvariable=s.c5).grid(row=4,column=1,padx=10,pady=10)
        u6=Label(f4,text="Washing Machines",bg="#074463",fg="lightgreen",font=("times new roman",16,"bold")).grid(row=5,column=0,padx=10,pady=10,sticky="w")
        ...
        u3=Label(f6,text="Total Electric Appliance Price",bg="#074463",fg="white",font=("times new roman",14,"bold")).grid(row=2,column=0,padx=20,pady=1,sticky="w")
        ...
        u6=Label(f6,text="Electric Appliance Tax",bg="#074463",fg="white",font=("times new roman",14,"bold")).grid(row=2,column=2,padx=20,pady=1,sticky="w")
    ...
    def total(s):
        ...
        s.q=s.c5.get()*3000
        s.r=s.c6.get()*15000
        s.total=float(s.m+s.n+s.o+s.p+s.q+s.r)
        s.d3.set("Rs. "+str(s.total))
        s.etax=round((s.total*0.05),2)
        s.f3.set("Rs. "+str(s.etax))
        
        
        s.Total_bill=float(s.totalcosmetic+s.totalgrocery+s.total+s.ctax+s.gtax+s.etax)
    ...
    def billarea(s):
        if s.cname.get()=="" or s.cphn.get()=="":
            ...
            if s.c1.get()!=0:
                s.txtarea.insert(END,f"\n AC \t\t {s.c1.get()}\t\t{s.m}")
            if s.c2.get()!=0:
                        s.txtarea.insert(END,f"\n Refrigerator \t\t  {s.c2.get()}\t\t{s.n}")
            if s.c3.get()!=0:
                        s.txtarea.insert(END,f"\n TV \t\t  {s.c3.get()}\t\t{s.o}")
            if s.c4.get()!=0:
                        s.txtarea.insert(END,f"\n water heaters  \t\t  {s.c4.get()}\t\t{s.p}")
            if s.c5.get()!=0:
                        s.txtarea.insert(END,f"\n microwave ovens \t\t  {s.c5.get()}\t\t{s.q}")
            if s.c6.get()!=0:
                        s.txtarea.insert(END,f"\n Washing Machines \t\t  {s.c6.get()}\t\t{s.r}")
            ...
            if s.f2.get()!="Rs. 0.0":
                s.txtarea.insert(END,f"\n Electric Appliance Tax \t\t\t {s.f3.get()}")
